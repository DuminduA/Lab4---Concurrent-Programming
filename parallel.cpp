...
double run_parallely(int size,double **matrix1, double **matrix2, double **result_matrix);
...
void calculate_time(int n, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        time_taken[i] = run_parallely(n, mat1, mat2, mat_res);
    }

    output_results(size, time_taken, "parallel");
...
#pragma omp parallel for shared(matrix1, matrix2, result_matrix)
    for (int i = 0; i < size; ++i) {
